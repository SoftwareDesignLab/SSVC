...
def predict_description(model_name, cve_description, working_dir=os.path.dirname(__file__), fold=1, confidence_interval=0.5):
    """
    The predict data function is a function that will load an already trained model, and open and process a csv
    CVE description and return the prediction.
    :param model_name: name of a trained model in the 'models' folder
    :param cve_description: description of cve being determined
    :param working_dir: the directory that should look for models (typically for use via import)
    :param fold: fold of the model to use
